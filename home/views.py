...
def index(request):
    ...
    four_most_liked_id = UserLike.objects.values('production_id').annotate(production_count=Count('production_id')).order_by('-production_count')[:4]
    four_most_liked = Production.objects.filter(pk__in=[item['production_id'] for item in four_most_liked_id])
